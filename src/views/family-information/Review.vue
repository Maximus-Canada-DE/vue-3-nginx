  ...
  <MainLayout>
    <template #main>
      ...
      <table class="table table-bordered">
        <tbody>
          <tr v-for="( [label, value], index ) in accountHolderFields " :key=" label + '-' + index ">
            <td>{{ capitalize(label) }}</td>
            <td>{{ value }}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="table table-bordered">
        <tbody>
          <tr v-for="( [label, value], index ) in spouseFields " :key=" label + '-' + index ">
            <td>{{ capitalize(label) }}</td>
            <td>{{ value }}</td>
          </tr>
        </tbody>
      ...
      <table class="table table-bordered" v-for="( child ) in children " :key=" child.id ">
        <tbody>
          <tr v-for="( [label, value], index ) in Object.entries(child) " :key=" label + '-' + index ">
            <td v-if="label !== 'id'" class="col-sm-6">{{ capitalize(label) }}</td>
            <td v-if="label !== 'id'" class="col-sm-6">{{ value }}</td>
          </tr>
        </tbody>
      </table>
    </template>
